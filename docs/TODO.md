...
- [x] 네이버 지도 연동 (MVP 2.2)
  ...
  ---
  - [x] **에러 사항 해결 (plan 모드 build)**
    - [x] 좌표 변환 함수 유효성 검증 추가 (`lib/types/tour.ts`)
      - [x] `convertKATECToWGS84` 함수에 0/NaN 체크 추가
      - [x] 한국 좌표 범위 검증 추가 (위도: 32.5~43.5°N, 경도: 123.5~132.5°E)
      - [x] 반환 타입을 `{ lng: number; lat: number } | null`로 변경
      - [x] 유효하지 않은 좌표는 `null` 반환 및 콘솔 경고
    - [x] NaverMap 컴포넌트 좌표 처리 개선 (`components/naver-map.tsx`)
      - [x] 좌표 변환 시 `null` 체크 및 필터링
      - [x] 좌표 변환 실패 시 콘솔 경고 메시지 추가 (관광지명, 원본 좌표 포함)
      - [x] 유효한 좌표가 없을 때 빈 상태 메시지 표시
      - [x] 중심 좌표 계산 검증 (한국 범위 내인지 확인)
      - [x] 유효하지 않은 중심 좌표면 서울 중심(37.5665, 126.9780)으로 설정
      - [x] 마커 생성 시 좌표 유효성 검증 추가
      - [x] 선택된 관광지로 지도 이동 시 좌표 유효성 검증 추가
      - [x] 호버된 관광지 마커 강조 시 좌표 유효성 검증 추가
    - [x] DetailMap 컴포넌트 좌표 처리 개선 (`components/tour-detail/detail-map.tsx`)
      - [x] 좌표 변환 시 `null` 체크 및 처리
      - [x] 좌표 변환 실패 시 콘솔 경고 메시지 추가
      - [x] 좌표가 `null`일 때 빈 상태 메시지 표시
      - [x] 지도 초기화 조건 개선 (`coordinates`가 `null`이 아닐 때만 초기화)
      - [x] 사용자 친화적 빈 상태 메시지 (원본 좌표 없음 vs 변환 실패 구분)
- [x] 페이지네이션
  - [x] 무한 스크롤 구현
...
- [ ] 배포 준비
  ...
  ---
  - [x] **추가 개발 내용 (plan 모드 build)**
    - [x] Vercel 배포 설정 확인 및 최적화
      - [x] `vercel.json` 파일 생성 (빌드 명령어, Node.js 버전, 프레임워크 설정)
      - [x] `package.json`에 `engines` 필드 추가 (Node.js >=18.0.0, pnpm >=8.0.0)
      - [x] `app/layout.tsx`에서 환경변수 검증 통합 확인 (이미 구현됨)
    - [x] 빌드 테스트
      - [x] 로컬 빌드 테스트 가이드 문서화 (`docs/DEPLOY_CHECKLIST.md`)
      - [x] 환경변수 검증 테스트 가이드 문서화
      - [x] 프로덕션 빌드 검증 가이드 문서화
      - [x] 빌드 에러 수정 가이드 문서화
    - [x] 프로덕션 배포 체크리스트
      - [x] `docs/DEPLOY_CHECKLIST.md` 생성 (배포 전/후 체크리스트)
      - [x] Vercel 환경변수 확인 항목 문서화 (필수/선택 환경변수 목록, 확인 방법)
      - [x] Git 배포 준비 확인 항목 문서화 (`.gitignore`, 브랜치, 변경사항 확인)
    - [x] 배포 후 검증 항목 문서화
      - [x] 배포 상태 확인 항목 (Vercel 대시보드, 빌드 로그, 배포 URL)
      - [x] 주요 기능 테스트 항목 (홈페이지, 상세페이지, 통계, 북마크, 인증)
      - [x] 성능 확인 항목 (페이지 로딩 속도, 이미지 최적화, API 응답 속도)
      - [x] 에러 모니터링 항목 (Runtime Logs, 브라우저 콘솔, API 에러)
      - [x] SEO 및 메타태그 확인 항목 (메타태그, Open Graph, Sitemap, Robots.txt)
      - [x] 문제 해결 가이드 (빌드 실패, 배포 후 에러, 성능 문제)
